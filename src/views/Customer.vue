        ...
        <el-col :span="24" style="padding-bottom: 0;">
            <el-form :inline="true" :model="filters">
                <el-form-item prop="name">
                    <el-input v-model="filters.name" placeholder="客户名称" prefix-icon="el-icon-search" @keyup.enter.native="btnSearch()"></el-input>
                </el-form-item>
                ...
                if (ids == '' || ids == null) {
                    ...
                    this.$confirm('确认删除选中记录吗？', '提示', {
                        ...
                        this.post_url(url, param, "删除客户失败！", null).then(function (res) {
                            ...
                            if (res.headers.code == Variable_global.errorCode.SUCCESS) {
                                that.$message.success("删除客户成功");
                                that.currentRow = {};
                                that.search();
                            }
                        }).catch(() => {
                            that.listLoading = false;
                            that.$message.error("删除客户失败");
                        });
                    }).catch(() => {
                        that.listLoading = false;
                        that.$message.error("删除客户失败");
                    ...
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            this.post_url(url, param, "新增客户失败！", null).then(function (res) {
                                ...
                                console.info(res);
                                if (res.headers.code == Variable_global.errorCode.SUCCESS) {
                                    that.$message.success("新增客户成功");
                                    that.addFormVisible = false;
                                    that.addForm = {};
                                    that.search();
                                } else {
                                    that.$message.error("新增客户失败");
                                }
                            }).catch(() => {
                                that.editLoading = false;
                                that.$message.error("新增客户失败");
