<template>
    <div style="height: 100%">
        ...
        <div v-if="nav.LOGO && this.nav.LOGO.id === '1'"
            ...
            <div>
                <a href="javascript:;">
                    <img :src="require('@/assets'+nav.LOGO.img)" alt=""
    ...
    export default {
        ...
        created() {
            ...
            Axios({
                ...
                for(let p = navs.length-1;p>=0;p--){
                    let child = navs[p].child;
                    if(child!=null && child.length>0){
                        for(let j = child.length-1;j>=0;j--){
                            if(authorities.indexOf(child[j].title) == -1){
                                child.splice(j,1);
                            }
                        }
                        if(child.length==0){
                        ...
                        if(authorities.indexOf(navs[p].title) == -1){
                            navs.splice(p,1);
                        }
                    }

                }
                console.info("相同权限", navs);
                res.data.nav = navs;
